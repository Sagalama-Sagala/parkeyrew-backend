...
export class FileUploadService {
  ...
  async uploadSingle(image: BufferedFile): Promise<string> {
    console.log("image in uploadSingle:",image);
  ...
  async uploadMany(images: {
    [key: string]: BufferedFile[];
  }): Promise<string[]> {
    ...
    if (Object.keys(uploadedImageUrls).length === 0) {
      throw new HttpException('No image', HttpStatus.INTERNAL_SERVER_ERROR);
    }
